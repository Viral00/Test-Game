...
namespace Checkers
{
    ...
    public class Core
    {
		...
		public void PrepareBoard(bool isPuzzle = false, Puzzle puzzle = null)
        {
        ...
        public void PreparePuzzle(Puzzle puzzle)
        {
            ...
            var whiteList = puzzle.Checkers.Where(x => x.Color == CheckerColor.White).ToList();
            for (int i = 0; i < whiteList.Count; i++)
            {
                var white = whiteList[i];
                index = (GameRule.Options.IsReverseBoard ? _boardSize - 1 - white.Pos.X : white.Pos.X) + white.Pos.Y * 8;

                int id = index + _idIncrement;
                Checker checker = new Checker(id, _squaresData[index], CheckerColor.White, white.IsCrown);
                _checkersData[id] = checker;
                _squaresData[index].SetChecker(checker);
            }

            var blackList = puzzle.Checkers.Where(x => x.Color == CheckerColor.Black).ToList();
            for (int i = 0; i < blackList.Count; i++)
            {
                var black = blackList[i];
                index = (GameRule.Options.IsReverseBoard ? _boardSize - 1 - black.Pos.X : black.Pos.X) + black.Pos.Y * 8;

                int id = index + _idIncrement;
                Checker checker = new Checker(id, _squaresData[index], CheckerColor.Black, black.IsCrown);
                _checkersData[id] = checker;
                _squaresData[index].SetChecker(checker);
            }
        }
        ...
        {
            bool isShouldBeatChecker = CheckBeatCheckerInAllDirections(checker.Id, checker.IsCrown && GameRule.Options.IsLongMoveAllow, colorForCheck);
            Debug.Log("IsshouldBeatChecker::" + isShouldBeatChecker);
            ...
            foreach (var direction in directions)
            {
                availableSquares.AddRange(GetAvailableSquaresInDirectionByDiagonal(checker,
                                                            ...
                                                            checker.IsCrown,
                                                            CheckBeatCheckerInDirection(checker.Id, checker.IsCrown && GameRule.Options.IsLongMoveAllow, direction, colorForCheck).Value,
                                                            isShouldBeatChecker,
        ...
        public List<Square> GetAvailableSquaresInDirection(Checker checker, PosDirection dir, CheckerColor colorForCheck)
        {
            ...
            return GetAvailableSquaresInDirectionByDiagonal(checker,
                                                            ...
                                                            GameRule.Options.IsLongMoveAllow,
                                                            checker.IsCrown,
                                                            CheckBeatCheckerInDirection(checker.Id, checker.IsCrown && GameRule.Options.IsLongMoveAllow, dir, colorForCheck).Value,
                                                            isShouldBeatChecker,
                                                            GameRule.Options.IsStillSimpleCheckerWhenBeatAfterLastRowAchieved,
                                                            GameRule.Options.IsNotRequiredBeating);
        }
        ...
        public List<Square> GetAvailableSquaresInDirectionByDiagonal(Checker checker, PosDirection dir, bool isDiagonalMove, bool isSuperChecker, bool isHasBeatCheckers, bool onlySquaresAfterBeating, bool isCanBeatBackLikeSimpleChecker, bool isNotRequiredBeating)
        {
            ...
            Debug.Log("issuperchecker" + (isSuperChecker && isDiagonalMove));
            foreach (var neighbourSquarePosition in checker.Square.NeighboursInDirection(isSuperChecker && isDiagonalMove, dir))
            {
                if (IsOnBoard(neighbourSquarePosition))
                {
                    ...
                    {
                        if (!checker.IsCrown && ((isCanBeatBackLikeSimpleChecker && !isHasBeatCheckers) || !isCanBeatBackLikeSimpleChecker && !isHasBeatCheckers))
                        {
                            continue;
                        }
                    }
                    ...
                    if (neighbourSquare != null)
                    {
                        ...
                        else
                        {
                            ...
                            else if (onlySquaresAfterBeating && isFoundSquareWithEnemyChecker)
                            {
                                ...
                                if (availableInDir != null && !availableInDir.HasChecker())
                                {
                                    Debug.Log("availableInDIr-3"+ availableInDir.Position.X.ToString()+":"+ availableInDir.Position.Y.ToString());
                                    isFoundSquareWithEnemyChecker = neighbourSquare.HasChecker();
                                    Debug.Log("issquarefoundwithchecker"+isFoundSquareWithEnemyChecker);
                                    if (isFoundSquareWithEnemyChecker)
                                    {
                                        allAvailableSquares.Add(availableInDir);
                                    }
        ...
        {
            if (choosenChecker.IsCrown && GameRule.Options.IsLongMoveAllow)
            {
            ...
            else
            {
                ...
                {
                    Debug.Log("IslongMoveAllowed:" + GameRule.Options.IsLongMoveAllow);
                    if (IsOnBoard(neighbourSquarePosition))
                    {
                        Square neighbourSquare = _boardSquares[neighbourSquarePosition.X, neighbourSquarePosition.Y];
        ...
        {
            Debug.Log("trytomovechecker" + squareId);
            ...
            if (_allAvailableSquares.Find(x => x.Id == square.Id) != null)
            {
                ...
                if (checker != null)
                {
                    ...
                    Square intermediateSquare = (checker.IsCrown && GameRule.Options.IsLongMoveAllow) ?
                ...
                if (!IsBeatProcessActive || !canBeatYet || isAchievedLastRow && !GameRule.Options.IsCanBeatAfterLastRowAchieve || IsBeatProcessActive && _currentChecker.IsCrown && GameRule.Options.IsShouldStopAfterDiagonalMove)
                {
                    _currentChecker = null;
        ...
        public List<Checker> FindBeatCheckers(CheckerColor colorForFindBeatChecker)
        {
            ...
            foreach (var checker in _checkersData.Values)
            {
                ...
                {
                    if (CheckBeatCheckerInAllDirections(checker.Id, checker.IsCrown && GameRule.Options.IsLongMoveAllow, colorForFindBeatChecker))
                    {
                ...
                foreach (var neighbourSquarePosition in checker.Square.NeighboursInDirection(diagonal, direction))
                {
                    if (IsOnBoard(neighbourSquarePosition))
                    {
                        ...
                        if (neighbourSquare.HasChecker() && neighbourSquare.Checker.Color != colorForCheck)
                        {
                            ...
                            {
                                if (!checker.IsCrown && !GameRule.Options.IsCheckersCanBeatBack)
                                {
            ...
            foreach (var checker in _checkersData.Values)
            {
                if (_currentChecker == null || checker == _currentChecker)
                {
                    if (checker.Color == colorForCheck && !checker.IsBeat)
                    {
                        foreach (var direction in directions)
                        {
                            CheckerBooleanResult result = CheckBeatCheckerInDirection(checker.Id, checker.IsCrown && GameRule.Options.IsLongMoveAllow, direction, colorForCheck);
                            if (result.Value)
                            {
        ...
        private bool IsCanCheckerBeatOtherCheckers(int checkerId)
        {
            ...
            return CheckBeatCheckerInAllDirections(checkerId, checker.IsCrown && GameRule.Options.IsLongMoveAllow, checker.Color);
        }
        ...
        {
            Debug.Log("GetSecpmdNextSquareInDirection");
